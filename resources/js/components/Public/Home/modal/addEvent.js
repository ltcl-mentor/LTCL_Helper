        ...
        axios
            ...
            .then(response => {
                if (response.status === 200) {
                    ...
                    props.onClose();
                    history.push("/?page=manage", { type: "event", status: "store" });
                }
            }).catch(error => {
                console.log(error);
            });
    ...
    return (
        <React.Fragment>
            ...
            <Box sx={{ width: "80%", m: '50px auto 0' }}>
                <Grid container sx={{ justifyContent: 'space-between', height: '70px' }}>
                    <Grid item sx={{ flexGrow: 1, height: '100%' }}>
                        <Typography component="p" sx={{ color: '#666666', fontSize: '20px', fontWeight: 'bold', mt: '14px' }}>
                            イベント名
                        </Typography>
                    </Grid>
