...
const questionForMentor = () => {
    ...
    const list = questions.eventList.map((question) => {
        ...
        if (question.check) {
            ...
            check = <Button variant="outlined" sx={stylePrivate}>非公開</Button>;
        }

        let status;
        if (question.status === 0) {
            status = <Button variant="contained" sx={styleStatusButton}>未対応</Button>;
        } else if (question.status === 1) {
            status = <Button variant="outlined" color="success" sx={styleStatusButton}>対応中</Button>;
        } else if (question.status === 2) {
            status = <Button variant="outlined" color="inherit" sx={styleStatusButton}>解決済</Button>;
        } else if (question.status === 3) {
            status = <Button variant="contained" color="error" sx={styleStatusButton}>要対応</Button>;
        }

        return (
            <div key={question.id}>
                <Link to={{pathname: `/questions/` + question.id, location: 'mentor'}}>
                        ...
                        <ListItemText
                            primary={
                                <Box sx={styleListItemText}>
                                    { check }
                                    { status }
                                    <Typography variant="body1" sx={styleQuestionTitle}>{question.title}</Typography>
                                </Box>
                            }
                        />
                    </ListItem>
        ...
        <div className="container">
            ...
            <Box sx={styleContents}>
                ...
                <Box>
                    ...
                    <Box display="flex" justifyContent="center" sx={stylePagination}>
                        { pagination }
                    </Box>
