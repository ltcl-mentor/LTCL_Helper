...
import { CategoryPC } from './form/contentPC';
import { KeywordPC } from './form/contentPC';
import { CategoryMobile } from './form/contentMobile';
import { KeywordMobile } from './form/contentMobile';
...
const condition = (props) => {
    ...
    let project;
    if (category == 0) {
        curriculum = <PurpleButton onClick={() => setCategory(0)} sx={styleButtonWidth}>カリキュラム</PurpleButton>;
        project = <WhiteButton onClick={() => setCategory(1)} sx={styleButtonWidth}>成果物</WhiteButton>;
    } else {
        curriculum = <WhiteButton onClick={() => setCategory(0)} sx={styleButtonWidth}>カリキュラム</WhiteButton>;
        project = <PurpleButton onClick={() => setCategory(1)} sx={styleButtonWidth}>成果物</PurpleButton>;
    }
    ...
    return (
        <div style={props.isWide ? styleMarginPC : styleMarginMobile}>
            ...
            <Typography sx={styleHeading}>
                2. トピック
                {props.isWide && <Typography sx={styleWarningPC} component="span">以下から1つを選択してください</Typography>}
            </Typography>
            {!props.isWide && <Typography sx={styleWarningMobile} component="span">以下から1つを選択してください</Typography>}
            <Topic 
                category={category}
                topic={topic}
                setTopic={setTopic}
            ...
            <Typography sx={styleHeading}>
                3. カリキュラム番号
                {props.isWide && <Typography sx={styleWarningPC} component="span">※ 任意です</Typography>}
            </Typography>
            {!props.isWide && <Typography sx={styleWarningMobile} component="span">※ 任意です</Typography>}
            <CurriculumNumber 
                category={category}
                topic={topic}
