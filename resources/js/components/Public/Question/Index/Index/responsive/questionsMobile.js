...
const styleGrayButton = {borderColor:'gray', color:'gray'};
const styleButtonMargin ={marginLeft: 2};
const styleButtonList = {display: 'flex', textAlign: 'flex-start'};
const styleQuestionTitle = {color: 'black', marginLeft: 2, marginTop: 0.7};
const styleSelector = { width:'60%',marginLeft: 2, marginTop: 2};
const stylePaginate = { marginTop: 1, marginBottom: 2, display: 'flex', justifyContent: "center" };

/**
 * カリキュラムの質問一覧
 */
function QuestionsPC(props) {
    ...
    const list = questions.eventList.map((question) => {
        ...
        if (question.check) {
            ...
            check = <Button　variant="outlined" sx={styleGrayButton}>非公開</Button>;
        }

        let status;
        if (question.status == 0) {
            status = <Button　variant="contained" sx={styleButtonMargin}>未対応</Button>;
        } else if (question.status == 1) {
            status = <Button　variant="outlined" color="success" sx={styleButtonMargin}>対応中</Button>;
        } else if (question.status == 2) {
            status = <Button　variant="outlined" color="inherit" sx={styleButtonMargin}>解決済</Button>;
        } else if (question.status == 3) {
            status = <Button　variant="contained" color="error" sx={styleButtonMargin}>要対応</Button>;
        }
        ...
        return (
                <div key={question.id}>
                    <Link to={{pathname: questionLink, location: 'question'}} /*target="_blank"*/>
                        <ListItem button sx={styleButtonList}>
                            <ListItemText
                                primary={
                                    <Box>
                                        {user.is_admin && check}
                                        {user.is_admin && status}
                                        <Typography　variant="h6"  sx={styleQuestionTitle}>{question.title}</Typography>
                                    </Box>
                                }
                            />
                        </ListItem>
