 ...
 * カリキュラム番号フォーム
 */
function CurriculumNumber(props) {
    ...
    useEffect(() => {
        ...
        if (props.category === 0 && props.topic <= 13) {
            setCurriculumNumbers(
                numbers[Number(props.category)][Number(props.topic)]
            );
        } else if (props.category === 1 && props.topic >= 14) {
            if (props.topic == 19) {
                setCurriculumNumbers(numbers[Number(props.category)][1]);
            } else {
                setCurriculumNumbers(numbers[Number(props.category)][0]);
            }
        } else {
            setCurriculumNumbers(numbers[0][0]);
        }
